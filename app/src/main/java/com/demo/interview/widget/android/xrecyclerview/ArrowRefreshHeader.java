package com.demo.interview.widget.android.xrecyclerview;
...
import com.demo.interview.widget.android.xrecyclerview.progressindicator.AVLoadingIndicatorView;
...
public class ArrowRefreshHeader extends LinearLayout implements BaseRefreshHeader {
    ...
    private LottieAnimationView lo_ainm1, lo_anim2;
    private TextView mHeaderTimeView;
    private LinearLayout mHeaderRefreshTimeContainer;
    private RelativeLayout layout_lottie, layout_default;
    ...
    private String TAG = "arrowhead";
    public final static int LAYOUTTYPE_DEFAULT = 1, LAYOUTTYPE_LOTTIE = 2;
    private int mLayouttype = 1;//当前展示布局模式 1，默认 2：lottie动画
    ...
    public ArrowRefreshHeader(Context context, int animtype) {
        super(context);
        this.mLayouttype = animtype;
        initView();
    }
    ...
    public void setStatusListener(onStateChange listener) {
    ...
    private void initView() {
        ...
        layout_default = findViewById(R.id.listview_header_content);
        layout_lottie = findViewById(R.id.layout_type2);
//        if (mLayouttype == LAYOUTTYPE_DEFAULT) {
...
//        } else
        if (mLayouttype == LAYOUTTYPE_LOTTIE) {

            layout_lottie.setVisibility(View.VISIBLE);
            layout_default.setVisibility(View.GONE);
            lo_ainm1 = findViewById(R.id.lottie_moving);
            lo_anim2 = findViewById(R.id.lottie_loading);
            lo_ainm1.setVisibility(VISIBLE);
        }
    ...
    public void setState(int state) {
        ...
        } else if (mLayouttype == LAYOUTTYPE_LOTTIE) {
            switch (state) {
                case STATE_NORMAL://最新状态为初始状态
                    if (mState == STATE_RELEASE_TO_REFRESH) {//之前的状态是放手就能刷新
                    }
                    if (mState == STATE_REFRESHING) {//之前的状态是刷新
                    }
                    break;
                case STATE_RELEASE_TO_REFRESH://最新状态 松手刷新
                    if (mState != STATE_RELEASE_TO_REFRESH) {// 非 松手刷新
                        if(listener!=null){
                            listener.status(STATE_NORMAL,STATE_RELEASE_TO_REFRESH);
                        }
                    }
                    break;
                case STATE_REFRESHING://最新状态 刷新中
                    lo_ainm1.setVisibility(INVISIBLE);
                    lo_anim2.setVisibility(VISIBLE);
                    lo_anim2.playAnimation();
                    break;
                case STATE_DONE://最新状态 刷新完成
                    lo_ainm1.setVisibility(VISIBLE);
                    lo_anim2.setVisibility(INVISIBLE);
                    lo_anim2.pauseAnimation();
                    break;
                default:
            }
        }
...
//                    if (listener != null) {
//                        listener.movepercent((getVisibleHeight() * 1f / mMeasuredHeight * 1f));
//                    }
                    setState(STATE_NORMAL);
                    if (mLayouttype == LAYOUTTYPE_LOTTIE) {
                        lo_ainm1.setProgress((getVisibleHeight() * 1f / mMeasuredHeight * 1f));
//                        lo_ainm1.playAnimation();
                    }
                }
            }
        }
    ...
    public interface onStateChange {
        void status(int oldtype, int newtype);

        void movepercent(float per);
